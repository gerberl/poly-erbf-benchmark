...
def compute_probe_regularity_v2(model, X_test, X_train, y_pred=None,
                                 alpha=0.1, m=50, k_local=10,
                                 n_anchors='auto', tau_multiplier=0.3,
                                 random_state=42):
    """
    Improved probe-based regularity metric (v2).

    Three improvements over v1 (compute_probe_regularity):
      1. Standardised probing space -- perturbations are isotropic in units
         of training-set standard deviation, regardless of original feature
         scales.
      2. Per-anchor local variance scaling -- perturbation magnitudes adapt
         to local data density via k-nearest-neighbour feature-wise std.
      3. Representative anchor selection -- k-means on standardised test
         points with adaptive count, rather than probing every test point.

    Parameters
    ----------
    model : fitted regressor with .predict()
    X_test : array (n_test, d)
        Test points to probe around.
    X_train : array (n_train, d)
        Training data, used for standardisation stats and clip bounds.
    y_pred : array (n_test,) or None
        Pre-computed predictions on X_test.  Computed from model if None.
    alpha : float
        Perturbation scale in units of local std (default 0.1).
    m : int
        Number of probes per anchor (default 50).
    k_local : int
        Neighbours for local variance estimation (default 10).
    n_anchors : int or 'auto'
        Number of anchor points.  'auto' = min(n_test, max(100, sqrt(n*d))).
    tau_multiplier : float
        tau = tau_multiplier * std(y_pred) for violation probability.
    random_state : int

    Returns
    -------
    dict with keys:
        lip_tail_ratio      : log10(q99/q75)
        lip_tail_ratio_q90  : log10(q99/q90)
        lip_q50 .. lip_q99  : quantiles of Lipschitz quotient distribution
        lip_mean, lip_std   : mean and std of quotients
        violation_prob       : fraction of probes with |dy| > tau
        tau                  : threshold used
        per_point_max_q95    : q95 of per-anchor max quotients
        n_probes             : total probe count
        n_anchors            : number of anchors used
        alpha                : perturbation fraction
        anchor_selection     : 'all' or 'kmeans'
        median_perturbation_dist : median L2 distance in standardised space
    """
    ...
    _nan_keys = [
        'lip_tail_ratio', 'lip_tail_ratio_q90',
        'lip_tail_ratio_band', 'lip_tail_ratio_multipair',
        'lip_q50', 'lip_q75', 'lip_q90', 'lip_q95', 'lip_q99',
        'lip_mean', 'lip_std', 'violation_prob', 'tau',
        'per_point_max_q95', 'n_probes', 'n_anchors', 'alpha',
        'anchor_selection', 'median_perturbation_dist',
    ]

    # --- 1. Standardise using training statistics ---
    train_mean = X_train.mean(axis=0)
    train_std = X_train.std(axis=0)
    safe_std = np.where(train_std > 1e-12, train_std, 1.0)

    Z_test = (X_test - train_mean) / safe_std
    Z_train = (X_train - train_mean) / safe_std

    # --- 2. Clip bounds in standardised space ---
    ...
    # --- 4. Anchor count ---
    if n_anchors == 'auto':
        n_anchors = min(n_test, max(100, round(np.sqrt(n_test * d))))
    n_anchors = min(n_anchors, n_test)

    # --- 5. Anchor selection ---
    if n_anchors >= n_test:
        anchor_idx = np.arange(n_test)
        anchor_selection = 'all'
    else:
        km = KMeans(n_clusters=n_anchors, n_init=3, max_iter=100,
                    random_state=random_state)
        km.fit(Z_test)
        # Pick closest real test point to each centroid
        nn_km = NearestNeighbors(n_neighbors=1).fit(Z_test)
        _, idx = nn_km.kneighbors(km.cluster_centers_)
        anchor_idx = np.unique(idx.ravel())
        n_anchors = len(anchor_idx)
        anchor_selection = 'kmeans'

    Z_anchors = Z_test[anchor_idx]        # (n_a, d)
    y_anchors = y_pred[anchor_idx]        # (n_a,)

    # --- 6. Local variance per anchor ---
    k_actual = min(k_local, n_test - 1)
    if k_actual < 1:
        return {k: np.nan for k in _nan_keys}

    nn_local = NearestNeighbors(n_neighbors=k_actual + 1).fit(Z_test)
    # Query for anchors (anchors are in Z_test, so first neighbour is self)
    _, nn_idx = nn_local.kneighbors(Z_anchors)
    nn_idx = nn_idx[:, 1:]  # exclude self

    # Per-anchor per-feature std from neighbours
    # Shape: (n_anchors, k_actual, d) -> std over axis=1 -> (n_anchors, d)
    neighbour_points = Z_test[nn_idx]  # (n_a, k_actual, d)
    local_stds = neighbour_points.std(axis=1)  # (n_a, d)

    # Fall back to 1.0 (global std in standardised space) for zero local variance
    local_stds = np.where(local_stds > 1e-12, local_stds, 1.0)

    # --- 7. Perturbation ---
    noise = rng.randn(n_anchors, m, d)  # (n_a, m, d)
    z_probes = Z_anchors[:, None, :] + alpha * local_stds[:, None, :] * noise

    # Clip to training support
    z_probes = np.clip(z_probes, z_min, z_max)

    # --- 8. Un-standardise and predict ---
    x_probes = z_probes * safe_std + train_mean  # (n_a, m, d)
    x_probes_flat = x_probes.reshape(n_anchors * m, d)
    y_probes = model.predict(x_probes_flat).reshape(n_anchors, m)

    # --- 9. Quotients (L2 in standardised space) ---
    z_diffs = z_probes - Z_anchors[:, None, :]   # (n_a, m, d)
    z_dists = np.linalg.norm(z_diffs, axis=2)    # (n_a, m)
    pred_diffs = np.abs(y_probes - y_anchors[:, None])  # (n_a, m)

    # Mask out zero-distance probes (from clipping collapse)
    valid = z_dists > 1e-15
    if not np.any(valid):
        return {k: np.nan for k in _nan_keys}

    quotients = np.full_like(z_dists, np.nan)
    quotients[valid] = pred_diffs[valid] / z_dists[valid]
    all_q = quotients[valid]
    ...
    # Epsilon scaled to q99 so tail ratio stays in a meaningful range even
    # for piecewise-constant models (DT) where q90 ≈ 0.  Caps the maximum
    # tail ratio at ~log10(100) ≈ 2.0 while being negligible for smooth models.
    eps_q = max(1e-10, 1e-2 * q99)
    tail_ratio_q75 = np.log10((q99 + eps_q) / (q75 + eps_q))
    tail_ratio_q90 = np.log10((q99 + eps_q) / (q90 + eps_q))

    # --- Robust tail ratio variants ---

    # Option B: Band-averaged tail ratio
    # mean(top 5%) / mean(80th-95th percentile) -- uses many data points
    # for both numerator and denominator instead of two point quantiles.
    upper_mask = all_q >= q95
    mid_mask = (all_q >= np.percentile(all_q, 80)) & (all_q < q95)
    if np.any(upper_mask) and np.any(mid_mask):
        band_num = np.mean(all_q[upper_mask])
        band_den = np.mean(all_q[mid_mask])
        eps_band = max(1e-10, 1e-2 * band_num)
        tail_ratio_band = np.log10((band_num + eps_band) / (band_den + eps_band))
    else:
        tail_ratio_band = np.nan

    # Option A: Multi-pair median tail ratio
    # Median of log10(q_high/q_low) across several quantile pairs,
    # each probing the tail-vs-upper-bulk contrast at a different depth.
    pairs = [(99, 90), (98, 88), (97, 86), (96, 84), (95, 82)]
    pair_ratios = []
    for hi, lo in pairs:
        qhi = np.percentile(all_q, hi)
        qlo = np.percentile(all_q, lo)
        eps_p = max(1e-10, 1e-2 * qhi)
        pair_ratios.append(np.log10((qhi + eps_p) / (qlo + eps_p)))
    tail_ratio_multipair = np.median(pair_ratios)

    tau = tau_multiplier * np.std(y_pred)
    violation_prob = np.mean(pred_diffs[valid] > tau)

    # Per-anchor max quotient (using nan-safe max)
    per_anchor_max = np.nanmax(quotients, axis=1)
    per_anchor_max = per_anchor_max[~np.isnan(per_anchor_max)]
    per_point_max_q95 = (np.percentile(per_anchor_max, 95)
                         if len(per_anchor_max) > 0 else np.nan)

    median_pert_dist = np.median(z_dists[valid])

    return {
        'lip_tail_ratio': tail_ratio_q75,
        'lip_tail_ratio_q90': tail_ratio_q90,
        'lip_tail_ratio_band': tail_ratio_band,
        'lip_tail_ratio_multipair': tail_ratio_multipair,
        'lip_q50': q50,
        'lip_q75': q75,
        'lip_q90': q90,
        'lip_q95': q95,
        ...
        'violation_prob': violation_prob,
        'tau': tau,
        'per_point_max_q95': per_point_max_q95,
        'n_probes': int(np.sum(valid)),
        'n_anchors': n_anchors,
        'alpha': alpha,
        'anchor_selection': anchor_selection,
        'median_perturbation_dist': median_pert_dist,
    }
...
def compute_model_smoothness_profile(model, X, y_pred=None, include_paths=False,
    ...
    dict with all smoothness metrics combined:
        ...
        violation_prob_001, _005, _01 : float
        lipschitz_median, _90, _99 : float
    ...
    profile.update({
        'violation_prob_01': viol_res.get('violation_prob_01', np.nan),
        'violation_prob_05': viol_res.get('violation_prob_05', np.nan),
        'violation_prob_1': viol_res.get('violation_prob_1', np.nan),
        'tau': viol_res['tau'],
    })
    ...
    profile.update({
        ...
        'lipschitz_90': lip_res['lipschitz_90'],
        'lipschitz_99': lip_res['lipschitz_99'],
    })
